...
exports.exportEtherpadAdditionalContent = (hookName, context, callback) =>
  callback(["comments"]);
...
exports.padCopy = async (hookName, context) => {
  await Promise.all([
    ...
    commentManager.copyCommentReplies(
      context.originalPad.id,
...
exports.socketio = (hookName, args, cb) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("addComment", async (data, respond) => {
      ...
      const [commentId, comment] = await commentManager.addComment(
        padId,
        content
      );
    ...
    socket.on("bulkAddComment", async (padId, data, respond) => {
      ...
      const [commentIds, comments] = await commentManager.bulkAddComments(
        padId,
        data
      );
    ...
    socket.on("bulkAddCommentReplies", async (padId, data, respond) => {
      padId = (await readOnlyManager.getIds(padId)).padId;
      const [repliesId, replies] = await commentManager.bulkAddCommentReplies(
        padId,
        data
      );
      socket.broadcast
        .to(padId)
        .emit("pushAddCommentReply", repliesId, replies);
    ...
    socket.on("updateCommentText", async (data, respond) => {
      try {
        ...
        const { padId } = await readOnlyManager.getIds(data.padId);
        await commentManager.changeCommentText(
          padId,
          commentId,
          commentText,
          authorId
        ...
        socket.broadcast
          .to(padId)
          .emit("textCommentUpdated", commentId, commentText);
        respond("");
      } catch (err) {
        respond(err.message || err.toString());
      }
    ...
    socket.on("addCommentReply", async (data, respond) => {
      const { padId } = await readOnlyManager.getIds(data.padId);
      const [replyId, reply] = await commentManager.addCommentReply(
        padId,
        data
      );
...
exports.padInitToolbar = (hookName, { toolbar, isReadOnly }, cb) => {
  if (isReadOnly !== false) {
    return cb();
  }
...
exports.eejsBlock_editbarMenuLeft = (hookName, args, cb) => {
  ...
  args.content += eejs.require(
    "ep_comments_page/templates/commentBarButtons.ejs"
  );
  return cb();
};
...
exports.clientVars = (hook, context, cb) => {
  const displayCommentAsIcon = settings.ep_comments_page
    ? settings.ep_comments_page.displayCommentAsIcon
    : false;
  const highlightSelectedText = settings.ep_comments_page
    ? settings.ep_comments_page.highlightSelectedText
    : false;
  return cb({
...
exports.expressCreateServer = (hookName, args, callback) => {
  args.app.get("/p/:pad/:rev?/comments", async (req, res) => {
    ...
    const padIdReceived = (
      await readOnlyManager.getIds(apiUtils.sanitizePadId(req))
    ).padId;
  ...
  args.app.post("/p/:pad/:rev?/comments", async (req, res) => {
    ...
    try {
      [commentIds, comments] = await commentManager.bulkAddComments(
        padIdReceived,
        data
      );
    } catch (err) {
  ...
  args.app.post("/p/:pad/:rev?/commentReplies", async (req, res) => {
    ...
    try {
      [replyIds, replies] = await commentManager.bulkAddCommentReplies(
        padIdReceived,
        data
      );
    } catch (err) {
